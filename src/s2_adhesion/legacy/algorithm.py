"""The original S2 cell aggregation detection algorithm, extracted verbatim.

Segmentation strategy: cell occupancy estimation (not boundary detection).
Pipeline per field: MIP -> normalize -> median blur -> FIXED threshold
-> binary_fill_holes -> morphology close -> connected components -> area
filter -> centre-Z-slice active-channel check.

This module is a line-for-line extraction of the pre-refactor
``detect_aggregations.py`` (see ``git show HEAD:detect_aggregations.py`` at
the commit this refactor started from). It exists so:

1. the root ``detect_aggregations.py`` script can become a thin wrapper that
   still behaves exactly as before for anyone mid-study, and
2. ``s2_adhesion.backends.legacy.LegacyThreshold2DBackend`` can drive the
   same pixel computation to emit ``ObjectRecord`` rows, so the old and new
   pipelines can be compared on identical data.

PRESERVATION, NOT IMPROVEMENT. Every behaviour below -- including ones that
are scientifically questionable -- is kept bit-for-bit identical to the
original script on purpose. Known limitations are called out in comments at
the point they occur, but never fixed here: fixing them would silently
invalidate any comparison against results already collected with the
original script. If a fix is ever wanted, it belongs in the new 3D pipeline,
not here.

``tests/unit/test_legacy_regression.py`` asserts this module's output is
IDENTICAL (not merely similar) to the pre-refactor script's output on
synthetic data -- that is the whole point of this extraction.

Only ``nd2``/``cv2``/``scipy``/``numpy`` are imported -- never ``torch`` or
``cellpose`` -- so this module (and everything that depends on it) stays
importable on a measurement-only machine with no ML stack installed.
``nd2`` is imported unconditionally at module level exactly as the original
script did, even though the rest of this package treats it as a lazy,
optional dependency (see ``s2_adhesion.io.nd2_source``) -- changing that
here would be an improvement, not a preservation, so it is left as-is.
"""
...
__all__ = [
    "CHANNEL_COLORS",
    "Config",
    "compute_area_threshold_px",
    "load_nd2",
    "iter_fields",
    "to_uint8",
    "max_projection",
    "compute_global_channel_minmax",
    "compute_median_kernel_size",
    "extract_channel_projections",
    "extract_center_slices",
    "merge_channels",
    "apply_pseudo_color",
    "pseudo_color_merge",
    "segment_occupancy",
    ...
    "detect_aggregations",
    "draw_contour_overlay_gray",
    "draw_contour_overlay_color",
    "save_debug_images",
    "process_field",
    "save_csv",
    "process_nd2",
...
def load_nd2(path: Path) -> tuple[np.ndarray, dict[str, int], float]:
    ...
    pixel_size_um : float
    """
    # Preserved limitation: only x spacing is read. A confocal Z step is
    # typically 5-10x coarser than the in-plane pixel; this 2D pipeline never
    # needed dz, but that also means "pixel_size_um" silently assumes square,
    # isotropic-in-plane pixels for the area-threshold conversion below. Kept
    # as-is deliberately for comparability with results already collected --
    # see s2_adhesion.io.nd2_source for the documented fix in the 3D pipeline.
...
def segment_occupancy(
    ...
    """
    # Preserved limitation: FIXED threshold, not adaptive (e.g. Otsu). The
    # original script had an Otsu variant commented out; kept commented out
    # here too, deliberately, for byte-identical behaviour.
...
def count_active_channels(
    ...
    Preserved limitation: activity is checked on the CENTRE Z-slice only, not
    the whole stack -- a channel that is active elsewhere in Z but not at the
    centre plane is scored inactive. Kept deliberately for comparability.
    """
    return sum(
...
def detect_aggregations(
    ...
    ``records`` is built by scanning ``label_id`` in ascending order (cv2's
    raster-scan connected-component label order) and appending only accepted
    regions in that same relative order, with ``aggregation_id`` a dense
    0-based counter over accepted regions only. Callers needing per-region
    geometry beyond ``RegionRecord`` (e.g. ``component_bboxes``) can rely on
    this ordering to line results up against ``valid_mask``.
    """
...
def binarize_slices(slices: list[np.ndarray], threshold: int) -> list[np.ndarray]:
    """Fixed-threshold binarize each slice.

    Factored out of the inline comprehension ``process_field`` used
    (``[cv2.threshold(slc, cfg.binary_threshold, 255, cv2.THRESH_BINARY)[1]
    for slc in center_slices]``) purely so other callers (e.g.
    ``s2_adhesion.backends.legacy``) don't have to duplicate it. Produces
    byte-identical output to that comprehension.
    """
    return [cv2.threshold(slc, threshold, 255, cv2.THRESH_BINARY)[1] for slc in slices]


def median_blur(gray: np.ndarray, kernel_size: int) -> np.ndarray:
    """Thin wrapper around ``cv2.medianBlur`` so callers outside this module
    don't need their own ``cv2`` import just for the one call
    ``process_field`` makes on the merged grayscale image."""
    return cv2.medianBlur(gray, kernel_size)


def component_bboxes(mask: np.ndarray) -> list[tuple[int, int, int, int, int]]:
    """Bounding box ``(x, y, w, h)`` and area for every foreground connected
    component in ``mask``, ordered by ascending label id (cv2's raster-scan
    order). New, additive helper -- not present in the original script.

    Used by callers that need per-region geometry ``RegionRecord`` does not
    carry (e.g. ``LegacyThreshold2DBackend`` computing ``touches_xy_border``).
    When called on the ``valid_mask`` returned by :func:`detect_aggregations`,
    this lines up 1:1, in order, with that call's ``records``: the accepted
    regions in ``valid_mask`` are exactly the same disjoint pixel sets as in
    the source ``mask``, so removing the rejected regions cannot change their
    relative raster-scan order, and ``detect_aggregations`` itself preserves
    that same order when building ``records``.
    """
    n_labels, _labeled, stats, _centroids = cv2.connectedComponentsWithStats(mask, connectivity=8)
    return [
        (
...
def process_field(
    ...
    gray_merged_median = median_blur(gray_merged, median_k)
    ...
    binary_center_slices = binarize_slices(center_slices, cfg.binary_threshold)
...
def main() -> None:
    ...
    # import time: a library consumer who only imports this module must not
    # have their stdout/stderr silently reconfigured. See _console.py --
    # every printed spacing/threshold below quotes 'µm', which crashes on a
    # cp932 (Japanese Windows) console without this.
